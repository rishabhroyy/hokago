...
- Hardware accel: no overlays — uncomment the `devices: /dev/dri` (Intel/AMD) or `gpus: all` (NVIDIA) block on both app services in `docker-compose.yml`. `HOKAGO_HWACCEL=auto` (already in the base env) detects whatever it can see; broken/absent GPU falls back to CPU.
