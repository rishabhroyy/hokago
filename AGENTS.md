...
- Scan parallelism: the scan walk probes files and ingests leaves with bounded pools (`PROBE_CONCURRENCY`/`INGEST_CONCURRENCY` in `packages/scanner/src/constants.ts`, default 8). Worker-side caps: `HOKAGO_ARTWORK_CONCURRENCY` (default 8, bounds ffmpeg), `HOKAGO_TRICKPLAY_CONCURRENCY` (default 8 — cheap since the per-tile rewrite, bounds parallel sheet jobs) and `HOKAGO_SCAN_CONCURRENCY` (default 2, parallel libraries).
