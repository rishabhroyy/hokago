...
- Transcode concurrency is capped per-API-process by `HOKAGO_MAX_TRANSCODES` (default 4); busy slots return 503 and clients retry. Idle sessions (no heartbeat 5 min) are reaped on a 60s sweep plus a boot sweep when the API starts.
- HLS segment length for the TRANSCODE tier is `HLS_SEGMENT_SECONDS` (`packages/ffmpeg/src/device-profile.ts`, 4s — mirrored as a literal in `apps/web/src/WatchPage.tsx` since it's Node-only, keep both in sync). It's the floor on start/seek-restart latency: a session's first segment can't exist until ffmpeg has encoded that many seconds of source, so this is the one knob that moves "how long until playback starts" for every TRANSCODE session regardless of hardware. Software (no hwaccel) encode speed itself is `HOKAGO_TRANSCODE_PRESET` (default `veryfast`, any libx264/libx265 preset name) — lower it on a weak CPU box; hardware encoders (vaapi/qsv/nvenc) ignore it, they have their own quality knobs.
- Scan parallelism: the scan walk probes files and ingests leaves with bounded pools (`PROBE_CONCURRENCY`/`INGEST_CONCURRENCY` in `packages/scanner/src/constants.ts`, default 8). Worker-side caps: `HOKAGO_FFMPEG_SPAWNS` (default 12 — global cap on live ffmpeg children across all artwork/trickplay jobs; without it a big trickplay wave can OOM the box and the SIGKILLed decodes read as file failures), `HOKAGO_ARTWORK_CONCURRENCY` (default 8, bounds ffmpeg), `HOKAGO_TRICKPLAY_CONCURRENCY` (default 8 — cheap since the per-tile rewrite, bounds parallel sheet jobs) with `HOKAGO_TRICKPLAY_TILE_CONCURRENCY` (default 4 — per-job tile fan-out; worst-case spawns = jobs × tiles, lower it for slow software decodes like 10-bit HEVC) and `HOKAGO_SCAN_CONCURRENCY` (default 2, parallel libraries).
...
- `packages/scanner` — evidence-based pipeline. Parser registry lives in `packages/scanner/src/parsers/` (anitomy for ANIME, scene regexes for GENERAL), noisy-OR confidence in `evidence.ts`. Known limitation (deferred): container-level confidence isn't computed yet, only leaves. **Show-scoped movies parent to the SERIES, not the root** (Bunny Girl Senpai shape): runtime outliers inside season dirs, `Movies/`-style subfolders, and loose files in a show folder all become MOVIE children of the series (anchor rule in `ingest.ts`) and render under the series detail's "Movies" section — only folders with no anchoring show become root-level movies. Scan re-runs are gated: files whose size+mtime match their stored row skip the partial-hash and stream re-derivation but still re-derive classification, so a plain rescan heals re-classifications. Unchanged files are re-probed only when the stored probe is untrustworthy — failed/empty probe, or an mp4/mov with zero subtitle tracks (the mov_text→TX3G backfill) — so a settled library still scans with zero ffprobes. Scan progress is reported per committed directory via `onScanProgress` → BullMQ job progress → the admin Libraries table.
...
- Detail-page descriptive fields (overview, originalTitle, rating 0–10, genres, studio) flow provider → `MetadataMatch` → `fillDescriptiveFields` (only when unset) → `MediaItemDetail` contract → DetailView. Tagline intentionally absent (no keyless provider exposes one). Manual "fix match" (`GET /metadata/search` + `POST /media-items/{id}/metadata-match` in `apps/api/src/metadata-routes.ts`, UI on the detail view): the pin writes the ExternalId + PROVIDER_MATCH evidence and enqueues the item's resolve job; a pinned/prior identity is never re-contested on title alone — `resolveMetadataStep` accepts the entry matching the held providerId even when `findAcceptedMatch` rejects it (providers revalidate `existingProviderId` by exact id lookup, so a rename can't miss the pin).
...
- **Done:** Step 0 license firewall (`packages/metadata` interfaces only) · Step 1 foundations (schema, contract, ffmpeg image, font subsets, compose) · Step 2 local-first scan pipeline (scan → group → NFO → embedded tags/art → generated art, run via `pnpm --filter @hokago/scanner scan <path>`, not yet job-queued) · Step 3 job infrastructure (reconciler, idempotency, checkpointing, graceful shutdown, admin console UI in the web app) · Step 4 parser registry + evidence engine + resolution + collections (parser profiles in `packages/scanner/src/parsers/`, noisy-OR confidence in `packages/scanner/src/evidence.ts`, `resolveMetadataStep` in `apps/worker`, `IdMapping.episodeOffset` + `Collection`/`CollectionEntry` in the schema) · Step 6 keyless network providers (TVmaze, AniList/Jikan, MAL — BullMQ queues + per-provider limiters in `apps/worker`) · Step 7 playback decision engine + HLS · Step 8 player (vidstack + JASSUB + fonts + track switching + COOP/COEP) · Step 9 auth, profiles, watch state, continue-watching + WS layer · dark mode (replaces the cut theming step) · REMUX tier (copy remux to fragmented MP4 for HEVC/h264 mkv — Chrome.s VideoToolbox decodes HEVC natively, so no re-encode; MSE.s HEVC gap is the reason this bypasses HLS entirely; the residual CPU transcode is the HDR/PGS/weird-codec path) · trickplay (sprite-sheet scrubber previews — 320x180/10s, 5x5 tiles, `/config/cache/trickplay/{fileId}/`, hash-gated regeneration via `Trickplay.sourceHash`; per-tile keyframe seeks, not whole-window decodes) · **hwaccel** (VAAPI/QSV/NVENC — compile-time support + userspace drivers in the image, `/dev/dri` mounted unconditionally so Intel/AMD is zero-config; NVIDIA = host toolkit + uncommented `gpus: all`, boot detection + fail-soft CPU fallback in `packages/ffmpeg/src/hwaccel.ts`, read-only status tile at `/admin-api/hwaccel` in the admin dashboard) · first-run setup wizard (admin account + libraries) · **release pipeline** (drop-in image-based compose template, `compose.build.yml` local-build overlay, `HOKAGO_JWT_SECRET` hard gate, version baked from the git tag into `/health` + admin sidebar, multi-arch CI/CD in `.github/workflows/`).
- **Next:** probe + fonts + subtitles + artwork store (eager font extraction, `.mks`, `fonts/`, PGS flagging) → segments (intro/outro skip) → watch parties → PUID/PGID (wizard step) → native-client store delivery (signing/notarization, deep links, TV rows — desktop shell is **done** (Tauri, thin webview over the web app, released by `native.yml` on every `v*` tag); iOS/Android/TV are being rebuilt natively in Flutter (`native/flutter`, in progress — see `docs/native-clients.md`), replacing the retired webview shells). Chromecast: **never**; AirPlay rides along with the native clients.
...
The full intent doc is `docs/native-clients.md` — read it before any native-client work. TL;DR: `apps/web` is the single UI source **and the player** (vidstack/JASSUB runs inside the webview — no native player, ever); native apps are thin webview shells + a narrow bridge (`packages/native-bridge`: `window.hokagoNative` with sync storage mirror, native downloads + `list`/`localUrl`/`readText`, version/identity). Shells live in `native/tauri`, `native/ios`, `native/android`; `.github/workflows/native.yml` builds them on every `v*` tag, bundling `apps/web/dist` into each for **offline mode** (offline library at `/offline`, `hokago-file://` local playback with Range, offline watch-progress flushed to `/watch-state/sync` on reconnect). The backend is wired: multi-account devices (`DeviceAccount`), TV pairing (`Device`/`PairingCode`), `HOKAGO_TRUST_PROXY` + `CF-Connecting-IP` for real-client-IP rate limiting, downloads (`Download` + `download` BullMQ queue, artifacts under `/config/downloads/<id>`), and offline watch-state sync. tvOS is dropped (no third-party WKWebView on tvOS). Update policy: bump `MIN_NATIVE_VERSION` (in `packages/native-bridge/src/versions.ts`) only for native-level changes.
