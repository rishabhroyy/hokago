...
- **Next:** probe + fonts + subtitles + artwork store (eager font extraction, `.mks`, `fonts/`, PGS flagging) → segments (intro/outro skip) → watch parties → PUID/PGID (wizard step) → native-client store delivery (signing/notarization, deep links, TV rows — the shells themselves are **done**: Tauri desktop, iOS WKWebView, Android phone+TV, all thin webview shells over the single web app, released by `native.yml` on every `v*` tag). Chromecast: **never**; AirPlay rides along with the native clients.
