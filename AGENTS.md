...
- `packages/scanner` — evidence-based pipeline. Parser registry lives in `packages/scanner/src/parsers/` (anitomy for ANIME, scene regexes for GENERAL), noisy-OR confidence in `evidence.ts`. Known limitation (deferred): container-level confidence isn't computed yet, only leaves. **Movies inside show folders stay inside the show** (Mugen Train shape): a runtime outlier that isn't episode-named is parented to the season like an episode, never root-level — only files in flat (non-season) directories become standalone movies. Scan progress is reported per committed directory via `onScanProgress` → BullMQ job progress → the admin Libraries table.
