...
The full intent doc is `docs/native-clients.md` — read it before any native-client work. TL;DR: `apps/web` is the single UI source **and the player** (vidstack/JASSUB runs inside the webview — no native player, ever); native apps are thin webview shells + a narrow bridge (`packages/native-bridge`: `window.hokagoNative` with sync storage mirror, native downloads, version/identity). Shells live in `native/tauri`, `native/ios`, `native/android`; `.github/workflows/native.yml` builds them on every `v*` tag. The backend is wired: multi-account devices (`DeviceAccount`), TV pairing (`Device`/`PairingCode`), `HOKAGO_TRUST_PROXY` + `CF-Connecting-IP` for real-client-IP rate limiting, downloads (`Download` + `download` BullMQ queue, artifacts under `/config/downloads/<id>`), and offline watch-state sync. tvOS is dropped (no third-party WKWebView on tvOS). Update policy: bump `MIN_NATIVE_VERSION` (in `packages/native-bridge/src/versions.ts`) only for native-level changes.
