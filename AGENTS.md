...
- `pnpm docker:up:vaapi` · `pnpm docker:up:nvenc` — compose with the hwaccel overlay baked in (`infra/hwaccel.transcoding.yml` / `infra/hwaccel.nvenc.yml`); still pulls the same image, no rebuild.
...
- `.env` holds compose-only vars (`HOKAGO_CONFIG_DIR`, `MEDIA_MOVIES_PATH`/`MEDIA_TV_PATH`/`MEDIA_ANIME_PATH`, `POSTGRES_PASSWORD`, `HOKAGO_JWT_SECRET`, ...); the app reads `DATABASE_URL`/`VALKEY_URL` from the environment.
...
- **`docker-compose.yml` is a drop-in template** (`.env.example` → `.env`): it pulls `ghcr.io/rishabhroyy/hokago:${HOKAGO_VERSION:-latest}`, has **no build blocks** (local builds go through the `compose.build.yml` overlay), and its only personalizations are path env vars — config, three media roots, ports. Library roots in the wizard are the FIXED container paths `/media/movies`, `/media/tv`, `/media/anime` that the mounts bind.
