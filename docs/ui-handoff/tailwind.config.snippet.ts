// Paste into the app's tailwind.config.{ts,js} under theme.extend.
// Keep token names aligned with design-system.md. Adjust to merge with existing config.

import type { Config } from "tailwindcss";

export const hokagoTheme = {
  colors: {
    paper: "#F6F0E6",
    "paper-2": "#EFE7D8",
    card: "#FFFFFF",
    ...
    wii: "#4FB8E0",
    "wii-2": "#8FE0F5",
    // per-title poster pastels (use via arbitrary gradients or a Poster component map)
...
} satisfies Config["theme"] extends infer T ? Partial<NonNullable<T>> : never;
